...
function getzsGrammar(getVariables: (grammarVariables: MapLike<string>) => MapLike<string>) {
    const grammarBeforeTransformation = readYaml(inputFile(Language.zs, Extension.YamlTmLanguage)) as TmGrammar;
...
function buildGrammar() {
    const grammar = getzsGrammar(grammarVariables => grammarVariables);

    // Write zs.tmLanguage
    fs.mkdirSync('syntaxes', { recursive: true });
    writePlistFile(grammar, outputFile(Language.zs, Extension.TmLanguage));
}
